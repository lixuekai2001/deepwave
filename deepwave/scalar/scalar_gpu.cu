...
static __constant__ DEEPWAVE_TYPE fd1[4 * DEEPWAVE_DIM];
static __constant__ DEEPWAVE_TYPE fd2[4 * DEEPWAVE_DIM + 1];
...
void setup(const DEEPWAVE_TYPE * const fd1_d,
  ...
  gpuErrchk(cudaMemcpyToSymbol(fd1, fd1_d, 4 * DEEPWAVE_DIM * sizeof(DEEPWAVE_TYPE)));
  gpuErrchk(cudaMemcpyToSymbol(fd2, fd2_d, (4 * DEEPWAVE_DIM + 1) * sizeof(DEEPWAVE_TYPE)));
}
